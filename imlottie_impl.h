...
#	define VECTOR_FALLTHROUGH
	...
	class vFlagHelper {
	  ...
	  public:
		...
		explicit constexpr inline vFlagHelper(int ai) noexcept
			: i(ai) {}
		...
		explicit constexpr inline vFlagHelper(uint ai) noexcept
			: i(int(ai)) {}

		explicit constexpr inline vFlagHelper(short ai) noexcept
			: i(int(ai)) {}

		explicit constexpr inline vFlagHelper(ushort ai) noexcept
			: i(int(uint(ai))) {}
	...
	enum LOTMaskType : unsigned char { MaskAdd = 0, MaskSubstract, MaskIntersect, MaskDifference };

	enum LOTMatteType : uchar { MatteNone = 0, MatteAlpha, MatteAlphaInv, MatteLuma, MatteLumaInv };
	...
	class vFlag {
	  public:
		...
		static_assert((sizeof(Enum) <= sizeof(int)), "vFlag only supports int as storage so bigger type will overflow");
		static_assert((std::is_enum<Enum>::value), "vFlag is only usable on enumeration types.");

		using Int = typename std::conditional<std::is_unsigned<typename std::underlying_type<Enum>::type>::value, unsigned int, signed int>::type;
		...
		constexpr vFlag(Enum f) noexcept
			: i(Int(f)) {}

		explicit constexpr vFlag(vFlagHelper f) noexcept
			: i(f) {}
	...
	class VColor {
	  public:
		...
		inline uchar red() const noexcept {
			return r;
		}

		inline uchar green() const noexcept {
			return g;
		}

		inline uchar blue() const noexcept {
			return b;
		}

		inline uchar alpha() const noexcept {
			return a;
		}

		inline void setRed(uchar red) noexcept {
			r = red;
		}

		inline void setGreen(uchar green) noexcept {
			g = green;
		}

		inline void setBlue(uchar blue) noexcept {
			b = blue;
		}

		inline void setAlpha(uchar alpha) noexcept {
			a = alpha;
		}
	...
	enum class BlendMode { Src, SrcOver, DestIn, DestOut };

#ifndef V_CONSTRUCTOR_FUNCTION
#	define V_CONSTRUCTOR_FUNCTION0(AFUNC)                                                                                                                                         \
		namespace {                                                                                                                                                                \
			static const struct AFUNC##_ctor_class_ {                                                                                                                              \
				inline AFUNC##_ctor_class_() {                                                                                                                                     \
					AFUNC();                                                                                                                                                       \
				}                                                                                                                                                                  \
...
#	define V_CONSTRUCTOR_FUNCTION(AFUNC) V_CONSTRUCTOR_FUNCTION0(AFUNC)
	...
	class VPointF {
	  public:
		...
		constexpr inline VPointF(float x, float y) noexcept
			: mx(x)
			, my(y) {}
	...
	class VPoint {
	  public:
		...
		constexpr inline VPoint(int x, int y) noexcept
			: mx(x)
			, my(y) {}
	...
	class VSize {
	  public:
		...
		constexpr inline VSize(int w, int h) noexcept
			: mw(w)
			, mh(h) {}
	...
	class VRect {
	  public:
		...
		VRect(int x, int y, int w, int h)
			: x1(x)
			, y1(y)
			, x2(x + w)
			, y2(y + h) {}

		explicit VRect(VPoint pt, VSize sz)
			: VRect(pt.x(), pt.y(), sz.width(), sz.height()) {}
		...
		bool contains(const VRect& r, bool proper = false) const {
			return proper ? ((x1 < r.x1) && (x2 > r.x2) && (y1 < r.y1) && (y2 > r.y2)) : ((x1 <= r.x1) && (x2 >= r.x2) && (y1 <= r.y1) && (y2 >= r.y2));
		}
...
#define TRANSMAP(x, y, nx, ny)                                                                                                                                                     \
	...
	class VMatrix {
	  public:
		...
		enum class MatrixType : unsigned char { None = 0x00, Translate = 0x01, Scale = 0x02, Rotate = 0x04, Shear = 0x08, Project = 0x10 };
		VMatrix() = default;
		...
		MatrixType type() const {
			...
			case MatrixType::Translate:
				...
				VECTOR_FALLTHROUGH
			case MatrixType::None: mType = MatrixType::None; break;
			}
		...
		inline float determinant() const {
			return m11 * (m33 * m22 - mty * m23) - m21 * (m33 * m12 - mty * m13) + mtx * (m23 * m12 - m22 * m13);
		}
		...
		VMatrix& translate(float dx, float dy) {
			...
			case MatrixType::Scale:
				...
				mty += dy * m22;
				break;
			case MatrixType::Project: m33 += dx * m13 + dy * m23; VECTOR_FALLTHROUGH
			case MatrixType::Shear:
			case MatrixType::Rotate:
				mtx += dx * m11 + dy * m21;
				mty += dy * m22 + dx * m12;
		...
		VRect map(const VRect& rect) const {
			...
			if (t <= MatrixType::Scale) {
				...
				xmax = vMax(xmax, x);
				ymax = vMax(ymax, y);
				return VRect(std::lround(xmin), std::lround(ymin), std::lround(xmax) - std::lround(xmin), std::lround(ymax) - std::lround(ymin));
			} else {
				// Not supported
				assert(0);
				return {};
		...
		bool fuzzyCompare(const VMatrix& o) const {
			return vCompare(m11, o.m11) && vCompare(m12, o.m12) && vCompare(m21, o.m21) && vCompare(m22, o.m22) && vCompare(mtx, o.mtx) && vCompare(mty, o.mty);
		}
	...
	class vcow_ptr {
	  ...
	  public:
		...
		template <class... Args>
		explicit vcow_ptr(Args&&...args)
			: mModel(new model(std::forward<Args>(args)...)) {}

		vcow_ptr(const vcow_ptr& x) noexcept
			: mModel(x.mModel) {
			assert(mModel);
			++mModel->mRef;
		}

		vcow_ptr(vcow_ptr&& x) noexcept
			: mModel(x.mModel) {
			assert(mModel);
			x.mModel = nullptr;
		}
	...
	class VRle {
	  public:
		...
		using VRleSpanCb = void (*)(size_t count, const VRle::Span *spans, void *userData);
	  ...
	  private:
		...
		friend void opIntersectHelper(const VRle::VRleData& obj1, const VRle::VRleData& obj2, VRle::VRleSpanCb cb, void *userData);
		vcow_ptr<VRleData> d;
	};
	...
	class VPath {
	  public:
		...
		void addRoundRect(const VRectF& rect, float roundness, VPath::Direction dir = Direction::CW);
		void addRect(const VRectF& rect, VPath::Direction dir = Direction::CW);
		void addPolystar(
			float points, float innerRadius, float outerRadius, float innerRoundness, float outerRoundness, float startAngle, float cx, float cy,
			VPath::Direction dir = Direction::CW
		);
		void addPolygon(float points, float radius, float roundness, float startAngle, float cx, float cy, VPath::Direction dir = Direction::CW);
	...
	inline void VPath::addRoundRect(const VRectF& rect, float roundness, VPath::Direction dir) {
	...
	inline void
	VPath::addPolystar(float points, float innerRadius, float outerRadius, float innerRoundness, float outerRoundness, float startAngle, float cx, float cy, VPath::Direction dir) {
		d.write().addPolystar(points, innerRadius, outerRadius, innerRoundness, outerRoundness, startAngle, cx, cy, dir);
	}

	inline void VPath::addPolygon(float points, float radius, float roundness, float startAngle, float cx, float cy, VPath::Direction dir) {
	...
	class VPathMesure {
	  public:
		...
		VPath trim(const VPath& path) {
			...
			if ((vCompare(mStart, 0.0f) && (vCompare(mEnd, 1.0f))) || (vCompare(mStart, 1.0f) && (vCompare(mEnd, 0.0f))))
				return path;
			...
			if (mStart < mEnd) {
				...
				float array[4] = {
					length * mEnd,
					(mStart - mEnd) * length, // 1st segment
					(1 - mStart) * length,
					std::numeric_limits<float>::max(), // 2nd segment
				};
				VDasher dasher(array, 4);
	...
	class VRasterizer {
	  public:
		...
		void rasterize(VPath path, CapStyle cap, JoinStyle join, float width, float miterLimit, const VRect& clip = VRect());
		VRle rle();
	...
	class VBitmap {
	  public:
		...
		enum class Format : uchar { Invalid, Alpha8, ARGB32, ARGB32_Premultiplied };
		...
		bool isNeedClear() const {
			return mImpl ? mImpl->mNeedClear : true;
		}

		void setNeedClear(bool needClear) {
			if (mImpl)
				mImpl->mNeedClear = needClear;
		}
	...
	struct VSpanData {
		...
		void setup(const VBrush& brush, BlendMode mode = BlendMode::SrcOver, int alpha = 255);
...
#define BYTE_MUL(c, a) ((((((c) >> 8) & 0x00ff00ff) * (a)) & 0xff00ff00) + (((((c) & 0x00ff00ff) * (a)) >> 8) & 0x00ff00ff))
...
#define LOOP_ALIGNED_U1_A4(DEST, LENGTH, UOP, A4OP)                                                                                                                                \
	...
	class VDrawable {
	  public:
		...
		enum class Type : unsigned char { Fill, Stroke, StrokeWithDash };
		...
		void setStrokeInfo(CapStyle cap, JoinStyle join, float miterLimit, float strokeWidth);
	...
	struct VImageLoader::Impl {
		...
		VBitmap load(const char *imageData, size_t len) {
			...
			int width, height, n;
			unsigned char *data = imageFromData((unsigned char *)imageData, static_cast<int>(len), &width, &height, &n, 4);
	...
	enum class LayerType : uchar { Precomp = 0, Solid = 1, Image = 2, Null = 3, Shape = 4, Text = 5 };
	...
	class LottieColor {
	  public:
		...
		LottieColor(float red, float green, float blue)
			: r(red)
			, g(green)
			, b(blue) {}

		VColor toColor(float a = 1) {
			return VColor(uchar(255 * r), uchar(255 * g), uchar(255 * b), uchar(255 * a));
		}
	...
	class LOTKeyPath {
	  ...
	  private:
		...
		bool isGlobstar(uint depth) const {
			return mKeys[depth] == "**";
		}

		bool isGlob(uint depth) const {
			return mKeys[depth] == "*";
		}
	...
	struct FrameInfo {
		explicit FrameInfo(uint32_t frame)
			: _frameNo(frame) {}
	...
	class LOTVariant {
	  public:
		...
		LOTVariant(imlottie::Property prop, const ValueFunc& v)
			: mPropery(prop)
			, mTag(Value) {
			construct(impl.valueFunc, v);
		}

		LOTVariant(imlottie::Property prop, ValueFunc&& v)
			: mPropery(prop)
			, mTag(Value) {
			moveConstruct(impl.valueFunc, std::move(v));
		}

		LOTVariant(imlottie::Property prop, const ColorFunc& v)
			: mPropery(prop)
			, mTag(Color) {
			construct(impl.colorFunc, v);
		}

		LOTVariant(imlottie::Property prop, ColorFunc&& v)
			: mPropery(prop)
			, mTag(Color) {
			moveConstruct(impl.colorFunc, std::move(v));
		}

		LOTVariant(imlottie::Property prop, const PointFunc& v)
			: mPropery(prop)
			, mTag(Point) {
			construct(impl.pointFunc, v);
		}

		LOTVariant(imlottie::Property prop, PointFunc&& v)
			: mPropery(prop)
			, mTag(Point) {
			moveConstruct(impl.pointFunc, std::move(v));
		}

		LOTVariant(imlottie::Property prop, const SizeFunc& v)
			: mPropery(prop)
			, mTag(Size) {
			construct(impl.sizeFunc, v);
		}

		LOTVariant(imlottie::Property prop, SizeFunc&& v)
			: mPropery(prop)
			, mTag(Size) {
			moveConstruct(impl.sizeFunc, std::move(v));
		}

		imlottie::Property property() const {
			return mPropery;
		}
	  ...
	  private:
		...
		void Move(LOTVariant&& other) {
			switch (other.mTag) {
			case Type::Value: moveConstruct(impl.valueFunc, std::move(other.impl.valueFunc)); break;
			case Type::Color: moveConstruct(impl.colorFunc, std::move(other.impl.colorFunc)); break;
			case Type::Point: moveConstruct(impl.pointFunc, std::move(other.impl.pointFunc)); break;
			case Type::Size: moveConstruct(impl.sizeFunc, std::move(other.impl.sizeFunc)); break;
			default: break;
			}
			mTag	   = other.mTag;
		...
		void Copy(const LOTVariant& other) {
			switch (other.mTag) {
			case Type::Value: construct(impl.valueFunc, other.impl.valueFunc); break;
			case Type::Color: construct(impl.colorFunc, other.impl.colorFunc); break;
			case Type::Point: construct(impl.pointFunc, other.impl.pointFunc); break;
			case Type::Size: construct(impl.sizeFunc, other.impl.sizeFunc); break;
			default: break;
			}
			mTag	 = other.mTag;
			mPropery = other.mPropery;
	...
	class LOTFilter {
	  ...
	  private:
		...
		const LOTVariant& data(Property prop) const {
			auto result = std::find_if(mFilters.begin(), mFilters.end(), [prop](const LOTVariant& e) {
	...
	class LOTAnimInfo {
	  public:
		...
		bool changed(int prevFrame, int curFrame) const {
			...
			return !((first > prevFrame && first > curFrame) || (last < prevFrame && last < curFrame));
		}
	...
	class LOTAnimatable {
	  public:
		...
		LOTAnimatable() {
			construct(impl.mValue, {});
		}

		explicit LOTAnimatable(T value) {
			construct(impl.mValue, std::move(value));
		}

		const LOTAnimInfo<T>& animation() const {
	...
	class LOTProxyModel {
	  public:
		...
		float miterLimit() const {
			return _modelData->miterLimit();
		}

		CapStyle capStyle() const {
			return _modelData->capStyle();
		}

		JoinStyle joinStyle() const {
			return _modelData->joinStyle();
		}

		bool hasDashInfo() const {
			return _modelData->hasDashInfo();
		}
	...
	class LOTData {
	  public:
		...
		enum class Type : unsigned char { Composition = 1, Layer, ShapeGroup, Transform, Fill, Stroke, GFill, GStroke, Rect, Ellipse, Shape, Polystar, Trim, Repeater };

		explicit LOTData(LOTData::Type type)
			: mPtr(nullptr) {
			mData._type		   = type;
			mData._static	   = true;
		...
		void setType(LOTData::Type type) {
			mData._type = type;
		}

		LOTData::Type type() const {
			return mData._type;
		}
	...
	class LOTFillData : public LOTData {
	  public:
		...
		LOTFillData()
			: LOTData(LOTData::Type::Fill) {}

		LottieColor color(int frameNo) const {
	...
	class LOTProxyModel<LOTFillData> {
	  public:
		...
		LOTProxyModel(LOTFillData *model)
			: _modelData(model) {}
	...
	class LOTGroupData : public LOTData {
	  public:
		...
		explicit LOTGroupData(LOTData::Type type)
			: LOTData(type) {}
	...
	struct TransformData {
		...
		void createExtraData() {
			if (!mExtra)
				mExtra = std::make_unique<TransformDataExtra>();
		}

		LOTAnimatable<float> mRotation{0}; /* "r" */
		LOTAnimatable<VPointF> mScale{
			{100, 100}
		};									/* "s" */
	...
	class LOTTransformData : public LOTData {
	  public:
		...
		LOTTransformData()
			: LOTData(LOTData::Type::Transform) {}
	...
	class LOTProxyModel<LOTGroupData> {
	  public:
		...
		LOTProxyModel(LOTGroupData *model = nullptr)
			: _modelData(model) {}

		bool hasModel() const {
			return _modelData ? true : false;
		}
		...
		LOTTransformData *transform() const {
			return _modelData->mTransform;
		}
	...
	enum class LottieBlendMode : uchar { Normal = 0, Multiply = 1, Screen = 2, OverLay = 3 };
			...
			if (isStatic()) {
				...
				for (const auto& keyFrame : vec) {
					if (frameNo >= keyFrame.mStartFrame && frameNo < keyFrame.mEndFrame) {
						LottieShapeData::lerp(keyFrame.mValue.mStartValue, keyFrame.mValue.mEndValue, keyFrame.progress(frameNo), path);
					}
				}
			}
		}
	...
	class LOTLayerData : public LOTGroupData {
	  public:
		...
		LOTLayerData()
			: LOTGroupData(LOTData::Type::Layer) {}
		...
		bool precompLayer() const {
			return mLayerType == LayerType::Precomp;
		}
	  ...
	  public:
		...
		ExtraLayerData *extra() {
			if (!mExtra)
				mExtra = std::make_unique<ExtraLayerData>();
			return mExtra.get();
		}
	...
	class LOTCompositionData : public LOTData {
	  public:
		...
		LOTCompositionData()
			: LOTData(LOTData::Type::Composition) {}
	...
	class LOTModel {
	  public:
		...
		double duration() const {
			return mRoot->duration();
		}

		size_t totalFrame() const {
			return mRoot->totalFrame();
		}

		size_t frameDuration() const {
			return mRoot->frameDuration();
		}

		double frameRate() const {
			return mRoot->frameRate();
		}

		size_t startFrame() const {
			return mRoot->startFrame();
		}

		size_t endFrame() const {
			return mRoot->endFrame();
		}

		size_t frameAtPos(double pos) const {
			return mRoot->frameAtPos(pos);
		}

		std::vector<LayerInfo> layerInfoList() const {
			return mRoot->layerInfoList();
		}
	...
	struct LOTKeyFrameValue<VPointF> {
		...
		VPointF value(float t) const {
			if (mPathKeyFrame) {
				 ...
				 */
				VBezier b = VBezier::fromPoints(mStartValue, mStartValue + mOutTangent, mEndValue + mInTangent, mEndValue);
	...
	class LOTShapeGroupData : public LOTGroupData {
	  public:
		...
		LOTShapeGroupData()
			: LOTGroupData(LOTData::Type::ShapeGroup) {}
	};
	...
	class LOTGradient : public LOTData {
	  public:
		...
		explicit LOTGradient(LOTData::Type type)
			: LOTData(type) {}
	...
	class LOTGFillData : public LOTGradient {
	  public:
		...
		LOTGFillData()
			: LOTGradient(LOTData::Type::GFill) {}
	...
	class LOTGStrokeData : public LOTGradient {
	  public:
		...
		LOTGStrokeData()
			: LOTGradient(LOTData::Type::GStroke) {}
	...
	class LOTPath : public LOTData {
	  public:
		...
		explicit LOTPath(LOTData::Type type)
			: LOTData(type) {}
	...
	class LOTTrimData : public LOTData {
	  public:
		...
		struct Segment {
			...
			explicit Segment(float s, float e)
				: start(s)
				, end(e) {}
		};
		...
		LOTTrimData::TrimType type() const {
			return mTrimType;
		}
	...
	class LOTRepeaterTransform {
	  public:
		...
		bool isStatic() const {
			return mRotation.isStatic() && mScale.isStatic() && mPosition.isStatic() && mAnchor.isStatic() && mStartOpacity.isStatic() && mEndOpacity.isStatic();
		}
	...
	class LOTRepeaterData : public LOTData {
	  public:
		...
		LOTRepeaterData()
			: LOTData(LOTData::Type::Repeater) {}

		LOTShapeGroupData *content() const {
			return mContent ? mContent : nullptr;
		}

		void setContent(LOTShapeGroupData *content) {
			mContent = content;
		}
	...
	class LottieLoader {
	  public:
		...
		bool loadFromData(std::string&& jsonData, const std::string& key, const std::string& resourcePath, bool cachePolicy);
		std::shared_ptr<LOTModel> model();
	...
	enum class DirtyFlagBit : uchar { None = 0x00, Matrix = 0x01, Alpha = 0x02, All = (Matrix | Alpha) };
	...
	class LOTClipperItem {
	  public:
		...
		explicit LOTClipperItem(VSize size)
			: mSize(size) {}
	...
	class VSpan {
	  public:
		...
		VSpan(pointer data, index_type size)
			: _data(data)
			, _size(size) {}
	...
	class LOTLayerItem {
	  public:
		...
		void setParentLayer(LOTLayerItem *parent) {
			mParentLayer = parent;
		}
		...
		bool hasMatte() {
			if (mLayerData->mMatteType == MatteType::None)
				return false;
			return true;
		}

		MatteType matteType() const {
	...
	class LOTCompLayerItem : public LOTLayerItem {
	  ...
	  private:
		...
		void renderMatteLayer(VPainter *painter, const VRle& inheritMask, const VRle& matteRle, LOTLayerItem *layer, LOTLayerItem *src);
	...
	enum class ContentType : uchar { Unknown, Group, Path, Paint, Trim };
	...
	class LOTContentItem {
	  public:
		...
		virtual ContentType type() const {
	...
	class LOTPathDataItem : public LOTContentItem {
	  public:
		...
		LOTPathDataItem(bool staticPath)
		...
		void setParent(LOTContentGroupItem *parent) {
			mParent = parent;
		}

		LOTContentGroupItem *parent() const {
			return mParent;
		}
	...
	class LOTRectItem : public LOTPathDataItem {
	  ...
	  protected:
		...
		bool hasChanged(int prevFrame, int curFrame) final {
			return (mData->mPos.changed(prevFrame, curFrame) || mData->mSize.changed(prevFrame, curFrame) || mData->mRound.changed(prevFrame, curFrame));
		}
	};
	...
	class LOTEllipseItem : public LOTPathDataItem {
	  ...
	  private:
		...
		bool hasChanged(int prevFrame, int curFrame) final {
			return (mData->mPos.changed(prevFrame, curFrame) || mData->mSize.changed(prevFrame, curFrame));
		}
	};
	...
	class LOTPolystarItem : public LOTPathDataItem {
	  ...
	  private:
		...
		bool hasChanged(int prevFrame, int curFrame) final {
			return (
				mData->mPos.changed(prevFrame, curFrame) || mData->mPointCount.changed(prevFrame, curFrame) || mData->mInnerRadius.changed(prevFrame, curFrame) ||
				mData->mOuterRadius.changed(prevFrame, curFrame) || mData->mInnerRoundness.changed(prevFrame, curFrame) || mData->mOuterRoundness.changed(prevFrame, curFrame) ||
	...
	enum LOTBrushType : uchar { BrushSolid = 0, BrushGradient };

	enum LOTFillRule : uchar { FillEvenOdd = 0, FillWinding };

	typedef enum { JoinMiter = 0, JoinBevel, JoinRound } LOTJoinStyle;

	enum LOTCapStyle : uchar { CapFlat = 0, CapSquare, CapRound };

	enum LOTGradientType : uchar { GradientLinear = 0, GradientRadial };
...
#define ChangeFlagAll	(ChangeFlagPath & ChangeFlagPaint)
	...
	template <>
	struct MapType<std::integral_constant<Property, Property::FillColor>> : Color_Type {};

	template <>
	struct MapType<std::integral_constant<Property, Property::StrokeColor>> : Color_Type {};

	template <>
	struct MapType<std::integral_constant<Property, Property::FillOpacity>> : Float_Type {};

	template <>
	struct MapType<std::integral_constant<Property, Property::StrokeOpacity>> : Float_Type {};

	template <>
	struct MapType<std::integral_constant<Property, Property::StrokeWidth>> : Float_Type {};

	template <>
	struct MapType<std::integral_constant<Property, Property::TrRotation>> : Float_Type {};

	template <>
	struct MapType<std::integral_constant<Property, Property::TrOpacity>> : Float_Type {};

	template <>
	struct MapType<std::integral_constant<Property, Property::TrAnchor>> : Point_Type {};

	template <>
	struct MapType<std::integral_constant<Property, Property::TrPosition>> : Point_Type {};

	template <>
	struct MapType<std::integral_constant<Property, Property::TrScale>> : Size_Type {};
